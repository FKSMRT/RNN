...
from tensorflow.keras.layers import Input, Dense, GRU, Conv1D, Lambda
...
OH_y = to_categorical(y, label_num)
OH_y_test = to_categorical(y_test, label_num)
...
decoded_LABEL = GRU(n_in)(decoded_rnn)
"""
decoded_LABEL = Dense(n_in, kernel_initializer=initializers,use_bias=False)(decoded_LABEL)
#decoded_LABEL = BatchNormalization()(decoded_LABEL)
decoded_LABEL = LeakyReLU(0.2)(decoded_LABEL)
"""
decoded_LABEL = Dense(label_num,kernel_initializer=initializers,activation = "sigmoid",name = "LABEL")(decoded_LABEL)
decoder = Model(latent_INPUT, [decoded_OUTPUT, decoded_LABEL], name = "decoder")
decoder.summary()


# Create the autoencodermodel
z_output = encoder(INPUT)[2]
OUTPUT = decoder(z_output)[1]
OUTPUTs = decoder(z_output)[0]
...
# Reconstruction Loss
reconstruction_loss = mse(INPUT, OUTPUTs)
...
autoencoder.fit(x, OH_y,
                epochs=epochs,
                batch_size=batch_size,
                shuffle=True,
                verbose = 1,
                validation_split = validation_split)
test_loss, test_accuracy = autoencoder.evaluate(x_test, OH_y_test, verbose = 0)
print("Test loss : ", test_loss)
print("Test accuracy :", test_accuracy)

# Generated waveform
_, _, z = encoder.predict(x)
Generated_waveform, _ = decoder.predict(z)
