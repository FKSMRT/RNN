...
# CSＶファイルをダウンロード
load_dataset = DataReader(dirpath,"\BPF_ch1_after\BPF_ch1_*.csv") # DataReader()を用いてCSVファイルを読み込む
...
print(x.shape)
"""
VF_split_labelset = [] # 分割リストの用意
for n in range(len(VF_label)): # 読み込んだ32のデータセットを20秒ごとに分割する
    VF_split_label = split_list(VF_label[n],split_num) 
    VF_split_labelset += [VF_split_label]
y = np.array(VF_split_labelset) # ndarray 化
print(y.shape)
